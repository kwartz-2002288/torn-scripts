...
def updateMAJ( wsName, gc, sheetKey, cell_range='A1:A2' ):                      
    from time import localtime, strftime
    ws = gc.open_by_key('1PDKlBarYILKPQzg_CQ_fGkHk8q8-DWmYeaUvaLqF_CA').worksheet( wsName )
    cell_list = ws.range( cell_range )
    cell_list[0].value = strftime("%I:%M:%S %p", localtime())
    ws.update_cells( cell_list )
    
def updateDate(gc,sheetKey):
    # put the update date on first page !
    ws = gc.open_by_key(sheetKey).worksheet( 'UserStats' )
    current_date = datetime.datetime.now().strftime("%d/%m/%Y %H:%M")
    ws.update_cell( 1,4,"Last update : " + current_date)
...
updateMAJ( "UserStats", gc, sheetKey)
updateMAJ( "FlowersAndPlushies", gc, sheetKey)
updateDate(gc,sheetKey)
